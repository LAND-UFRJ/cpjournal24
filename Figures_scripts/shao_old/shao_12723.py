...
fig = plt.figure(constrained_layout=True, figsize=(4.5,3.7))
...
ax['legend'].axis('off')
ax[0].set_title('Labeled changepoints')
...
ax[1].set_title('Basic methods and Pelt-NP')
...
for i,m in enumerate(others_ps_list):
    ...
    for cp in CP:
        ...
        if m != 'vwcd':
            ...
            ax[2].axvline(cp, color='b', linestyle='-', alpha=0.5, linewidth=0.5)
            ax[2].plot(cp, y0+i*d, 
                       marker=markers_dict[method_name(m)], 
                       markersize=4, 
                       color='b')
